...
**What you do.** Open the `0_general_chatbot` flow. The canvas shows three nodes — Chat Input on the left, **General Chatbot** in the middle, Chat Output on the right. Click **Playground**. A chat panel opens. You can use this exactly the way you would use ChatGPT or Gemini or Claude:

- Type a question and press send. You'll get an answer.
- Click the paperclip icon next to the input box to attach one or more files (PDFs, text files, markdown, etc.), then type a question about the attached files and press send.

For the suggested workshop exercise, attach all four Omar AML component PDFs at once and ask *"Produce an integrated diagnostic report for this patient using all four reports."* But you don't have to do that — you can also attach just one of the four and ask about that single report, or attach none and ask a general question. It's a chatbot; it answers whatever you ask.
...
|---|---|---|
| Chat Input | _(paperclip in Playground)_ | Attach zero, one, or many files inline with your message — same UX as ChatGPT or Gemini. The General Chatbot reads any attachments off the message and parses them in-process (PDF, DOCX, txt, markdown, csv, json, yaml, xml, html). For the suggested integrated-report exercise, attach the four Omar AML PDFs from `data/scenario_d/case_aml/` in the workshop repo. |
