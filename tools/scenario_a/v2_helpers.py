"""Shared helpers for Scenario A v2 LangFlow components.

Mirrors the structure of tools/scenario_c/v2_helpers.py — same OTEL
registration, same OpenAI client wired to KeyBroker, same chat_completion
wrapper. Kept per-scenario for now to keep each scenario self-contained;
common-ization can wait until we see real divergence.
"""
...
for _root in _CANDIDATES:
    if (Path(_root) / "tools" / "scenario_a").exists() and _root not in sys.path:
        sys.path.insert(0, _root)
        break


DEFAULT_DATA_DIR = "/workshop/data/scenario_a"
    ...
    *,
    ...
    if span_name:
        try:
            ...
            tracer = trace.get_tracer("workshop.scenario_a")
